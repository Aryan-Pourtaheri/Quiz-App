...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1e293b] via-[#3b82f6] to-[#fbbf24] flex flex-col">
      <Navbar />
      <main className="flex flex-1 flex-col items-center justify-center pt-32 pb-16 px-4 text-black">
        <div className="bg-white/90 rounded-xl shadow-lg p-8 w-full max-w-md flex flex-col items-center">
          ...
          <form onSubmit={(e) => handleLoginSubmit(e, user,resetForm)} className="w-full flex flex-col gap-4">
            <input type="email" placeholder="Email" className="px-4 py-2 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#3b82f6]" onChange={(e) => setUser({ ...user, email: e.target.value })} value={user.email} />
            <input type="password" placeholder="Password" className="px-4 py-2 rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#3b82f6]" onChange={(e) => setUser({ ...user, password: e.target.value })} value={user.password}/>
